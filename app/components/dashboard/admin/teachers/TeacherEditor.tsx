...
// Define the TeacherEditor component
export default function TeacherEditor({
  ...
  // Fetch available users who are not already teachers
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        // Filter out users who are already teachers (unless we're editing this teacher)
        const response2 = await fetch('/api/teachers');
        if (!response2.ok) {
          throw new Error(`Failed to fetch teachers: ${response2.status}`);
        }
        
        const teachers = await response2.json();
        const teacherUserIds = teachers.map((t: any) => t.user.id);
        
        // If we're editing a teacher, include their user in the list
        let filteredUsers;
        if (teacher) {
          filteredUsers = allUsers.filter((user: any) => 
            user.id === teacher.user.id || !teacherUserIds.includes(user.id)
          );
        } else {
          filteredUsers = allUsers.filter((user: any) => 
            !teacherUserIds.includes(user.id)
          );
        }
  ...
  const validateForm = () => {
    ...
    if (!formData.bio) {
      newErrors.bio = translations.errors.bioRequired;
    }
    
    if (locale === 'fa' && !formData.bioFa) {
      newErrors.bioFa = translations.errors.bioFaRequired;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-4">
          {teacher ? translations.editTeacher : translations.newTeacher}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              disabled={teacher !== null || loadingUsers}
            >
          ...
          <div>
            ...
            <textarea
              ...
              dir="rtl"
              className={`w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 ${
                errors.bioFa ? 'border-red-500' : 'border-gray-300'
              }`}
            />
            {errors.bioFa && (
              <p className="text-sm text-red-500 mt-1">{errors.bioFa}</p>
            )}
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              {translations.fields.specialties}
            </label>
            <input
              type="text"
              name="specialties"
