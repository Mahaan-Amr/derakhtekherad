...
export async function POST(request: Request) {
  console.log('==== BLOG UPLOAD API REQUEST STARTED ====');
  ...
  try {
    ...
    const filename = `blog_${timestamp}_${randomString}${path.extname(file.name)}`;

    // Save in the /public/uploads/blog directory for direct access
    const publicUploadDir = path.join(process.cwd(), 'public', 'uploads', 'blog');
    ...
    const fileUrl = `/uploads/blog/${filename}?v=${Date.now()}`;

    console.log(`[Upload] Blog image saved successfully: ${fileUrl}`);
    ...
    console.log('==== BLOG UPLOAD API REQUEST COMPLETED ====');
  }
} 